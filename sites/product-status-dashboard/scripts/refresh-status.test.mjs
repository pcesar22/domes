...
test('premature completion cannot cross an unmet dependency',()=>{const m=clone();m.nodes.find(n=>n.id==='FS-WP-004A').state='Active';m.nodes.find(n=>n.id==='FS-WP-004B').state='Complete';assert.throws(()=>validate(m,align(m)),/Unsatisfied prerequisite/);});
test('active execution cannot bypass an unmet prerequisite',()=>{const m=clone();m.nodes.find(n=>n.id==='FS-WP-004A').state='Active';m.nodes.find(n=>n.id==='FS-WP-004B').state='Active';assert.throws(()=>validate(m,align(m)),/Unsatisfied prerequisite/);});
