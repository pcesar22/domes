...
The protocol layer between master and slave pods is to be designed in a future document. This section captures the high-level categories and requirements.

### Message Categories

The protocol needs to support these categories of communication:

- **Discovery** — Master finds nearby pods, pods identify themselves
- **Session management** — Join/leave a game session, assign roles
- **Clock synchronization** — Align pod clocks for coordinated timing (sequence drills)
- **Actuator commands** — Control LEDs, audio, haptic on target pods
- **Sensor control** — Arm/disarm touch detection with timeout windows
- **Event reporting** — Pods report touch events, timeouts, errors back to master
- **Health/status** — Battery, RSSI, firmware version, pod state

### Protocol Requirements

1. **Fits ESP-NOW constraints** — Max 250 bytes per message, connectionless, no guaranteed delivery
2. **Common header** — All messages share a versioned header for type dispatch and sequencing
...
6. **No acknowledgment required for commands** — Fire-and-forget for actuator commands (LED, sound, haptic). The master does not wait for ACKs.
7. **Events are best-effort** — Touch events should be retried (2-3 attempts) since they carry scoring data, but the protocol does not guarantee delivery
8. **Protobuf or packed structs** — Encoding format TBD. Must be efficient on ESP32 (no dynamic allocation in the receive path)

### Detailed message definitions, encoding format, and sequence diagrams are deferred to a future protocol design document.
