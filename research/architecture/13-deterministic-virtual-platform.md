...
> **Document status: Active architecture; bounded QEMU target execution is implemented.** The
> existing host simulator remains the deterministic functional model. FS-WP-002B established QEMU
> viability and FS-WP-002D delivered the production-runtime composition. The virtual radio device,
> scheduler-causality campaigns, and predictive qualification remain future work. Delivery state and
> authorization are recorded in [`../../PROGRAM_STATUS.md`](../../PROGRAM_STATUS.md). Bounded target
> execution does not establish hardware equivalence or predictive trust.
...
1. The existing native host simulator remains the fast multi-pod functional and fault-exploration
   tier.
2. The pinned Espressif QEMU is the bounded target-execution tier for the ESP32-S3 Xtensa image,
   IDF FreeRTOS SMP, task affinities, target time, and supported interrupt delivery. Complete
   scheduler-switch and causality observability remains gated by FS-WP-002C.
...
The following must remain production-owned in the first radio-capable vertical slice delivered
after the D, E, and F packages:
...
- the **QEMU root** initially initializes trace and infrastructure, deterministic profile
  identity/config, feature and mode management, declared peripheral adapters, and the supported
  production game/runtime services and task topology. D keeps radio disabled; F later adds
  `EspNowTransport` and `EspNowService` after E establishes the production radio seam; and
...
FS-WP-002D binds this contract in
`firmware/domes/profiles/runtime_profiles.json`. The strict build generator emits a canonical
manifest and C++ profile header from the resolved SDKCONFIG. Each component and task state resolves
through a named fidelity contract containing implementation, inputs, outputs, timing, calibration,
and limitations; empty or unknown contracts fail configuration. Both roots consume the generated
initialization sequence at runtime, and build validation rejects any difference between the declared
and source-bound stage order. The physical and QEMU ELFs embed their manifest, specification,
SDKCONFIG, and required-task configuration hashes.

The QEMU root executes the versioned `service_ready_v1` synthetic workload. It must observe exactly
one GameEngine hit on the scripted pad, zero misses, and a return to `READY`. It must also observe
one entry handshake for every required task, reject duplicate entry, and report exact, disjoint
per-core masks after the declared target-time dwell. Pinned tasks must appear on their declared core;
unpinned tasks must appear on exactly one core. Final-ELF validation also denies linked entry points
for disabled WiFi, ESP-NOW, BLE, OTA, serial, and TCP paths. These entry handshakes prove task entry
and reject duplicate initialization only; they are not context-switch, blocking, ISR,
synchronization, or causality traces. That broader evidence remains FS-WP-002C.

The profile specification calls the single readiness marker `authenticated` in a bounded integrity
sense: the runner accepts exactly one marker and binds its fields to the canonical fidelity manifest,
resolved SDKCONFIG, required-task configuration, and profile specification. The accepted report
separately binds source and build artifact identities around that marker. There is no key, signature,
MAC, attestation root, or remote-origin assertion. This term must not be used as a cryptographic
provenance or trust claim.

A behavioral runner or analysis-contract change invalidates target acceptance. A fail-closed
pre-run validator-only hardening may retain immutable runtime results only when a recorded impact
analysis proves there is no firmware-source change, the exact accepted build passes the corrected
validator, and a regression captures the escaped input. Otherwise the target campaign must rerun.
...
- Account separately for every configured/compiled source-closure and actual linked-member difference
  from the `main` base, restrict those differences to the intentional composition/input
  refactor, preserve physical init order, enabled features, and two-board behavior, and prove no QEMU
  implementation is reachable from the physical image. Byte-identical source maps are not expected
  when shared assembly is extracted.
