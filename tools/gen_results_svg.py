...
def main():
    ...
    # Header
    parts.append(
        f'<text x="{width//2}" y="28" text-anchor="middle" fill="#e6edf3"'
    ...
    parts.append(
        f'<text x="{width//2}" y="48" text-anchor="middle" fill="#8b949e" font-size="11">'
    ...
    for label, start, count, color in phases:
        ...
        parts.append(
            f'<rect x="2" y="{y1}" width="4" height="{y2 - y1}" rx="2" fill="{color}" opacity="0.6"/>'
        )
        mid_y = (y1 + y2) / 2
        parts.append(
            f'<text x="10" y="{mid_y + 4}" fill="{color}" font-size="8"'
    ...
    for ms in range(0, 301, 50):
        ...
        parts.append(
            f'<line x1="{xp:.1f}" y1="{margin_top - 10}" x2="{xp:.1f}"'
            f' y2="{margin_top + len(rounds) * (bar_height + bar_gap)}"'
            f' stroke="#21262d" stroke-width="0.5"/>'
        )
        parts.append(
            f'<text x="{xp:.1f}" y="{margin_top - 15}" text-anchor="middle"'
    ...
    for i, (pod, hit, react_ms, pad, phase) in enumerate(rounds):
        ...
        if hit:
            ...
            if bar_w > 80:
                ...
                parts.append(
                    f'<text x="{margin_left + bar_w - 8}" y="{y + bar_height // 2 + 4}"'
                    f' text-anchor="end" fill="#0d1117" font-size="10">'
                    f"{react_ms:.0f}ms</text>"
                )
            else:
                parts.append(
                    f'<text x="{margin_left + bar_w + 5}" y="{y + bar_height // 2 + 4}"'
            ...
            # X marks
            parts.append(
                f'<text x="{margin_left + chart_width - 10}"'
    ...
    parts.append(
        f'<text x="{width//2}" y="{summary_y}" text-anchor="middle"'
    ...
    for pod in sorted(pod_stats.keys()):
        ...
        parts.append(
            f'<text x="{sx}" y="{summary_y + 20}" fill="{color}" font-size="10">'
