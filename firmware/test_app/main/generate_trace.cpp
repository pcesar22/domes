 ...
 *
 * Build: cmake -S firmware/test_app -B firmware/test_app/build
 *        cmake --build firmware/test_app/build --target trace_generator
 * Run:   firmware/test_app/build/trace_generator [output.json]
 */
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < kNumRounds; i++) {
        ...
        steps.push_back({r.target, r.delayMs, 3000, 0x03, domes::Color::rgb(r.r, r.g, r.b)});
    ...
    printf("Hits:      %zu\n", result.hitCount());
    printf("Misses:    %zu\n", result.missCount());
    printf("Avg react: %u us (%.1f ms)\n", result.avgReactionUs(), result.avgReactionUs() / 1000.0);
    ...
    for (size_t i = 0; i < result.rounds.size(); i++) {
        ...
        printf("  [%2zu] Pod %u: %s", i, r.targetPodId, r.hit ? "HIT " : "MISS");
        if (r.hit) {
            printf(" react=%u us (%.1f ms) pad=%u", r.reactionTimeUs, r.reactionTimeUs / 1000.0,
                   r.padIndex);
        }
        printf("\n");
    }
    ...
    bool ok = PerfettoExporter::exportToFile(outputPath, sim::globalTraceEvents(), sim.log(),
